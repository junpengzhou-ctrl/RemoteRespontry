...
import com.njaccp.myreg.login.LoginServicelmpl;
...
public class Test {
    public static void main(String[] args) throws Exception {
        TextDataSourceHandlerImpl dsh = new TextDataSourceHandlerImpl();
        ...
        while(true){
            ...
            if(opt==1){
                ...
                boolean login = ls.login(usr);
                if(login){
                    //判断是男女
                    int isnv = ls.isnv(usr.getUsername(), usr.getPassword());
                    System.out.println("尊贵的"+usr.getUsername()
                            +(isnv== 1 ?"先生":"女士")+"登录成功");
                    break;
                }else{
