...
(function ($) {
    $.extend({
        ...
        operate: {
            ...
            trans: function(id){
                table.set();
                $.modal.confirm("确定将该条" + table.options.modalName + "转到出库吗？", function(){
                    ...
                    if ($.common.isEmpty(id) && table.options.type == table_type.bootstrapTreeTable) {
                        ...
                        var url = table.options.newCreateUrl.replace("{id}", row[table.options.uniqueId]);
                        $.modal.open(table.options.newName, url);
                    } else {
                        $.modal.open(table.options.newName, $.operate.stepUrl(id));
                    }
                });
            },
            ...
            stepUrl: function(id) {
                var url = "/404.html";
                if ($.common.isNotEmpty(id)) {
                    url = table.options.newCreateUrl.replace("{id}", id);
                } else {
                    var id = $.common.isEmpty(table.options.uniqueId) ? $.table.selectFirstColumns() : $.table.selectColumns(table.options.uniqueId);
                    if (id.length == 0) {
                        $.modal.alertWarning("请至少选择一条记录");
